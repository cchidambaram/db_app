...
vector <string> Update_Content_Offset;
vector <string> Select_Update_Content;
/* Global Variables */
...
// Update function
bool update_function (string command ,catalog *catalog_object)
{
	string table_name,char_size;
	bool table_exists_flag=false;
	int table_position,char_size1;
	string temp_cmd = command;
	temp_cmd.erase(0, temp_cmd.find(' ')+1);
	string temp_cmd1 = temp_cmd.substr(0,temp_cmd.find(' '));
	temp_cmd1 = remove_spaces ( temp_cmd1);
	table_name =temp_cmd1;
	char char_value[100];
	...
	temp_cmd.erase(0, temp_cmd.find(' ')+1);
	string temp_cmd2 = temp_cmd.substr(0,3);
	temp_cmd2= remove_spaces ( temp_cmd2);
	if( temp_cmd2 != "set")
	{
		cout<<"Syntax Error: Set Keyword is missing\n";
		output.push_back("Syntax Error: Set Keyword is missing\n");
		return 0;
	}
	temp_cmd.erase(0, temp_cmd.find("set")+3);

	// Checking whether update column is present
	string update_condition,update_column,update_value;
	update_condition = temp_cmd.substr(0,temp_cmd.find("where"));
	update_condition = remove_spaces (update_condition);
	if(update_condition.find("=") != string::npos  )
	{
		update_column = update_condition.substr(0, update_condition.find("="));		
	}			
	update_column = remove_spaces(update_column);
	//Checking whether the column in given condition is present
	bool isValidcolumn = false;
	int Update_column_position;
	for (unsigned i = 0;i<catalog_tbl[table_position].col_name.size();i++)
	{
		if(catalog_tbl[table_position].col_name[i] == update_column)
			{
				Update_column_position = i;				
				isValidcolumn = true;				
				update_condition.erase(0,update_condition.find("=")+1);
				update_value = update_condition.substr(0, update_condition.size());
				update_value = remove_spaces(update_value);
			}
			
	}
	if( ! isValidcolumn )
	{
			cout<< " Column Name in update set is not present \n " ;
			output_table.push_back(" Column Name in update set is not present\n");
			return 0;
	}


	//Where Condition
	string cond1,col1,value1;
	
	temp_cmd.erase(0, temp_cmd.find("where")+6);
	cond1 = temp_cmd;
	cond1 = remove_spaces(cond1);
		...
		if(catalog_tbl[table_position].col_name[i] == col1)
			{
				isValidcolumn1_1 = true;
				column1_position = i;				
				cond1.erase(0,cond1.find("=")+1);
				value1 = cond1.substr(0, cond1.size());
	...
	ifstream update; bool is_clause;
	int col_int;
	vector <string> col_values;
	string table_name1 = table_name.append(".tbl");	
	
	const char * c = table_name1.c_str();		
	update.open (c,ios::in|ios::binary|ios::ate);
	if(update.is_open())
	{
		update.seekg (0, ios::end);
		unsigned size = update.tellg();
				...
				{					
					is_clause= false;
						for (unsigned i = 0;i<catalog_tbl[table_position].col_name.size();i++)
						{
							if (catalog_tbl[table_position].col_type[i] == "int")
							{
								update.read((char *)(&col_int),4);
								cout.width(15);
								cout.setf ( ios_base::left);								
								if(catalog_tbl[table_position].col_name[i] == col1 && col_int == atoi(value1.c_str()))
									is_clause = true;	
								stringstream convert; 
								convert << col_int;
								string col_int = convert.str();								
							...
							else if (catalog_tbl[table_position].col_type[i].substr(0,4) == "char")
							{
								...
								char_size.erase(char_size.size()-1);
								char_size1 = atoi(char_size.c_str());								
								update.read((char *)(&char_value),char_size1);
								cout.width(15);
								cout.setf ( ios_base::left);
								char_value[char_size1] = '\0';
								
								
									if(catalog_tbl[table_position].col_name[i] == col1 && char_value == value1.substr(1,value1.size()-1))
									is_clause = true;
						...
						if (is_clause)
						{
							int offset_columns = 0;
								...
								// Actual Update content to be stored in vector
								for ( unsigned km = 0;km<Update_column_position;km++ )
								{
									if (catalog_tbl[table_position].col_type[km] == "int")
									{
										offset_columns += 4;
									}
									else if (catalog_tbl[table_position].col_type[km].substr(0,4) == "char")
									{
									char_size = catalog_tbl[table_position].col_type[km].substr(5,catalog_tbl[table_position].col_type[km].size()-1);
									char_size.erase(char_size.size()-1);
									offset_columns += atoi(char_size.c_str());
									}
								}
								
								int offset= j + offset_columns;
								string update_type;
								if ( update_value.find("+") != string::npos)
								{
									// set a=a+1 scenario
									string before_addition, after_addition;
									before_addition = update_value.substr(0,update_value.find("+"));
									after_addition =  update_value.substr(update_value.find("+")+1 , update_value.size());
									update.seekg(offset);
									update.read((char *)(&col_int),4);
									int update_value_int = col_int + atoi(after_addition.c_str());
									stringstream convert; 
									convert << update_value_int;
									update_value = convert.str();
									
								}
								stringstream convert1; 
								convert1 << offset;
								string offset_str = convert1.str();	
								stringstream convert2; 
								convert2 << table_position;
								string table_position_str = convert2.str();

								update_type = "int";
								if( update_value.find("'") != string::npos)
								{
									update_type = "char";
									update_value = update_value.substr(1,update_value.size()-2);
									string update_size = catalog_tbl[table_position].col_type[Update_column_position].substr(5,catalog_tbl[table_position].col_type[Update_column_position].size()-1);
									update_size.erase(update_size.size()-1);
									Update_Content_Offset.push_back( table_position_str + ";" + offset_str + ";" + update_value + ";" + update_type + ";" + update_size );
								}
								else
								{
									Update_Content_Offset.push_back( table_position_str + ";" + offset_str + ";" + update_value + ";" + update_type );
								}
								// Checking wheather the datatype is valid for update column
								if ( update_type != catalog_tbl[table_position].col_type[Update_column_position])
								{
									cout<< " Data type specified in update column does not match \n " ;
									output_table.push_back("Data type specified in update column does not match \n");
									return 0;
								}

						string col_values1;
						// Col values should be updated in shared memory for other programs to acccess.
						for ( unsigned i = 0 ; i < col_values.size() ; i ++ )
						{
							if ( i == Update_column_position )
								col_values1 += update_value +";";
							else
							col_values1 += col_values[i] + ";" ;
							 
						}
						Select_Update_Content.push_back(col_values1);
...
// To check if the process is already running
...
void Begin_Transaction (string command , catalog *catalog_object)
{
	
	string temp_cmd = remove_spaces(command);
	string commit_or_rollback ; 
	temp_cmd.erase(0, temp_cmd.find(' ')+1);
	string temp_cmd1 = temp_cmd.substr(0,temp_cmd.find(';'));
	temp_cmd1 = remove_spaces ( temp_cmd1);
	if(temp_cmd1 != "transaction")
	{
		cout<<"Syntax error:Transaction keyword Error\n";
		output.push_back("Syntax error:Transaction keyword Error\n");		
	}
	temp_cmd.erase(0, temp_cmd.find(';')+1);
	vector <string> query;
	split(temp_cmd,';',query);
	for(int i =0; i< query.size(); i++)
	{
		
		query[i] = remove_spaces(query[i]);
	}
	if(query[query.size()-1] == "" )
		query.pop_back();
	commit_or_rollback = query[query.size()-1];
	query.pop_back();
	
	for(int j=0; j < query.size() ; j++)
	{
		int position;
		string command_line_argument = remove_spaces(query[j]);
		//SELECT QUERY
		if (command_line_argument.substr(0, command_line_argument.find(' ')) == "select")
		{
			bool clause =false;
			for ( int k =0 ; k < j ; k++ )
			{
				string temp_cmd_line_arg = remove_spaces(query[k]);
				if ( temp_cmd_line_arg.substr(temp_cmd_line_arg.find("where")+6,temp_cmd_line_arg.size()) == command_line_argument.substr(command_line_argument.find("where")+6,command_line_argument.size()))
				{
					position = k;
					clause = true;
				}
		...
		if ( clause )
		{
			char * Shared = "dbms.exe";		
			bool s = IsProcessRunning(Shared);
			if (s && count_of_processes> 1)
			{
			Sleep (1000*15);
			}
			vector <string> Update_Content ;
			split(Update_Content_Offset[position],';',Update_Content);	
			int table_position = atoi(Update_Content[0].c_str());
			
			// Select stmt within the begin transaction has same where clause as previous
			output_table.push_back(catalog_tbl[table_position].table_name+"\n");
			for (unsigned i = 0;i<catalog_tbl[table_position].col_name.size();i++)
			{
			...
			output_table.push_back("\n");
			// Select 
			vector<string> Update_Contents;
			split( Select_Update_Content[position],';',Update_Contents);
			for ( int i = 0 ; i < Update_Contents.size() ; i++)
			{
				output_table.push_back(Update_Contents[i]);
				output_table.push_back("\t");
			}
		...
		// UPDATE QUERY
		if (command_line_argument.substr(0, command_line_argument.find(' ')) == "update")
		{
		 bool isSuccess =update_function(command_line_argument,catalog_object);
		 if(! isSuccess)
			{
				cout<<"Update Statement Not executed \n" ;
				output.push_back("Update Statement Not executed \n");
			}
		}

	}
	commit_or_rollback = remove_spaces ( commit_or_rollback);
	// Commit Transaction or RollBack
	if ( commit_or_rollback == "commit" )
	{
		

	for ( unsigned k = 0 ; k < Update_Content_Offset.size() ; k++)
	{
		vector <string> Update_Content ;
		split(Update_Content_Offset[k],';',Update_Content);	
		int table_position = atoi(Update_Content[0].c_str());
		string table_name =  catalog_tbl[table_position].table_name;
		fstream update;
		string table_name1 = table_name.append(".tbl");	
		const char * c = table_name1.c_str();		
		update.open (c, ios::in |  ios::out | ios::binary);
		if(update.is_open())
		{			
			int offset = atoi(Update_Content[1].c_str());
			update.seekp(offset);
			if ( Update_Content[3] == "char")
			{
				const char * c = Update_Content[2].c_str();
				update.write((char *)(c),atoi(Update_Content[4].c_str()));
			}
			else
			{
			int int_value = atoi(Update_Content[2].c_str());
			update.write((char *)(&int_value),4);
			}
			update.close();
		}
		output.push_back("Committed Successfully\n");	
	}
	}
	else if ( commit_or_rollback == "rollback")
	{
		Update_Content_Offset.erase(Update_Content_Offset.begin(),Update_Content_Offset.end());	
		Select_Update_Content.erase(Select_Update_Content.begin(),Select_Update_Content.end());	
		output.push_back("Rollbacked successfully \n");
	}
	else
	{
		Update_Content_Offset.erase(Update_Content_Offset.begin(),Update_Content_Offset.end());	
		Select_Update_Content.erase(Select_Update_Content.begin(),Select_Update_Content.end());
		output.push_back("No Rollback or Commit \n");
	}
...
void main(int argc, char* argv[])
{
	...
	string command_line_argument_1;
	
	if (argc == 1)
	{		
		cout << "Enter Script File \n";
		getline (cin, command_line_argument_1);
		command_line_argument = command_line_argument_1;		
	}
	else
	{
		command_line_argument_1 = argv[1];
	...
	//string command_line_argument_1 = "script";
	...
	if (command_line_argument_1.substr(0,6) == "script")
	{
		...
		temp_cmd_line_arg=remove_spaces(temp_cmd_line_arg);
		 ...
		 string begin_query;int j;

		
		 for(int i = 0; i < query.size(); i++)
			{	
				 
			string query1 = remove_spaces(query[i]);
			 if ( query1.substr(0, query1.find(' ')) == "begin")
			 {
				 for(j=i; j < query.size() ; j++)
				 {
					 query[j] = remove_spaces(query[j]);
					 begin_query += query[j] + ";" ;					 
					 if( remove_spaces(query[j]) == "commit" || remove_spaces(query[j]) == "rollback" || j == (query.size()-1) )
					 {
						 Begin_Transaction(begin_query,catalog_object);
						 break;
					 }
